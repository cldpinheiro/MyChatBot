...
ChatLogic::~ChatLogic()
{
    ...
    // delete all nodes [not needed anymore]
    // for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    // {
    //     delete *it;
    // }

    // delete all edges [not needed anymore]
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            (*childNode)->AddEdgeToParentNode(edge.get()); // need raw pointer
                            (*parentNode)->AddEdgeToChildNode(std::move(edge)); // need smart pointer
                        }
